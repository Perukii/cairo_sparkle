...
private:
    bool transform_basis = casp_transform_basis_w;
...
public:
    ...
    template<class... Args> casp_surface(Args...);
...
template<class... Args> casp_surface::casp_surface(Args... args){ setup_surface(args...); }

casp_surface::~casp_surface(){
    cairo_destroy(cr);
}

void casp_surface::setup_surface
...
void casp_surface::set_wh() {
    xywh.w = (transform_basis == casp_transform_basis_w)
                ? scale
                : scale / resolution.y * resolution.x;

    xywh.h = (transform_basis == casp_transform_basis_h)
                ? scale
                : scale / resolution.x * resolution.y;
}

void casp_surface::set_color(casp_rgb _color){
    cairo_set_source_rgba(cr, _color.r, _color.g, _color.b, _color.a);
}

void casp_surface::draw_background(casp_rgb _color) {
    cairo_set_source_rgba(cr, _color.r, _color.g, _color.b, _color.a);
    cairo_rectangle(cr, 0, 0, resolution.x + 1, resolution.y + 1);
    cairo_fill(cr);
}

double casp_surface::transform_x(double _tar_x) {
    return (_tar_x - xywh.x) *
           (transform_basis == casp_transform_basis_w ? resolution.x : resolution.y)
           / scale + resolution.x * 0.5;
}

double casp_surface::transform_y(double _tar_y) {
    return (_tar_y - xywh.y) *
           (transform_basis == casp_transform_basis_w ? resolution.x : resolution.y)
           / scale + resolution.y * 0.5;
}

double casp_surface::transform_w(double _tar_w) {
    return _tar_w *
            (transform_basis == casp_transform_basis_w ? resolution.x : resolution.y)
            / scale;
}

double casp_surface::transform_h(double _tar_h) {
    return _tar_h *
            (transform_basis == casp_transform_basis_w ? resolution.x : resolution.y)
            / scale;
}

double casp_surface::re_transform_x(double _tar_x){
    return scale * (_tar_x - resolution.x * 0.5) /
                (transform_basis == casp_transform_basis_w ? resolution.x : resolution.y)
                 + xywh.x;
}

double casp_surface::re_transform_y(double _tar_y){
    return scale * (_tar_y - resolution.y * 0.5) /
                (transform_basis == casp_transform_basis_w ? resolution.x : resolution.y)
                 + xywh.y;
}

casp_xy<double> casp_surface::transform_xy(casp_xy<double> _tar_xy){
...
casp_xy<double> casp_surface::transform_wh(casp_xy<double> _tar_wh){
    casp_xy<double> result;
...
casp_xywh<double> casp_surface::transform_xywh(casp_xywh<double> _tar_xywh){
