...
public:
     ...
     double translate_x(double _tar){
          return (_tar-xy.x)*
               (translate_norm == casp_translate_norm_w ? scale.x:scale.y)/range*zoom+scale.x*0.5;
     }

     double retranslate_x(double _tar){
          return range*(_tar-scale.x*0.5)
               /(translate_norm == casp_translate_norm_w ? scale.x:scale.y)/zoom+xy.x;
     }

     double translate_y(double _tar){
          
          return (_tar-xy.y)*
               (translate_norm == casp_translate_norm_w ? scale.x:scale.y)/range*zoom+scale.y*0.5;
          
     }

     double translate_w(double _tar){
          return _tar*
               (translate_norm == casp_translate_norm_w ? scale.x:scale.y)/range*zoom;
     }
