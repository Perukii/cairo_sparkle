...
void setup_game(){
     ...
     stage .setup_rect(0.0, 0.5, surface.scale_range.x-1.0, surface.scale_range.y, casp_rgb_null);
     ...
     for(int iy=0;iy<board_y;iy++){
          for(int ix=0;ix<board_x;ix++){
               blocks[iy][ix].setup_rect(
                    0.55-surface.scale_range.x*0.5+(stage.xywh.w/board_x*ix)*0.99,
                    0.6-surface.scale_range.y*0.5+0.78*iy,
                    stage.xywh.w/board_x,0.75,casp_rgb(0.4, 0.4, 0.4, 1.0), 0, 0);
...
bool sphere_collision_all(){
     ...
     return sphere_collision(&stage , sphere.xywh) == true  and
            (sphere_collision(&player, sphere.xywh) == false or player.xywh.x >= sphere.xywh.y);
}
...
void casp_main(){
     ...
     player.xywh.x = host.mouse_pos.x;
     ...
     if(times == 0){
          ...
          if(host.get_button_press(1) == true) times = 1;
     }
     ...
     if(host.get_key_press(32)){
          host.write_png("/home/tada/Documents/Codes/CASP/cairo_sparkle/test.png");
     }
...
void casp_draw(){
     ...
     for(int iy=0;iy<board_y;iy++){
          for(int ix=0;ix<board_x;ix++){
               if(blocks[iy][ix].isbroken == false) blocks[iy][ix].draw_rect();
               else{
                    if(blocks[iy][ix].xywh.x != 0) broken++;
     ...
     if(broken == board_x*board_y){
          casp_text("CONGRATULATIONS !",-6.0, -4.5, casp_auto, 1.2, text_color, 0.0, 0.0).draw_text();
          if(host.get_button_press(1) == true) setup_game();
     }
}
